...
export default function AlertsPage() {
  ...
  return <AuthGuard><AdminShell><Typography.Title level={2}>提醒日志</Typography.Title><Table rowKey="id" dataSource={data?.items ?? []} onRow={(record) => ({ onClick: () => router.push(`/alerts/${record.id}`), className: "cursor-pointer" })} pagination={{ current: page, pageSize, total: data?.total ?? 0, showSizeChanger: true, showTotal: (total) => `共 ${total} 条`, onChange: (nextPage, nextSize) => { setPage(nextPage); setPageSize(nextSize); } }} columns={[{ title: "ID", dataIndex: "id" }, { title: "用户", dataIndex: "user_id" }, { title: "Stock", dataIndex: "stock_id" }, { title: "信号", dataIndex: "signal_id" }, { title: "标题", dataIndex: "title" }, { title: "状态", dataIndex: "status" }, { title: "发送时间", dataIndex: "sent_at" }]} /></AdminShell></AuthGuard>;
}
