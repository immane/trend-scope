    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {entries.map(([month, value]) => (
        <div key={month} className="rounded-lg border border-slate-200 bg-white p-3">
          <div className="mb-2 flex items-center justify-between text-sm"><span>{month.slice(0, 7)}</span><strong className={value >= 0 ? "text-emerald-600" : "text-rose-600"}>{percentText(value)}</strong></div>
  ...
  return (
    <AuthGuard><AdminShell>
      <Space direction="vertical" size="large" className="w-full">
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="策略总收益率" value={percentText(data?.total_return)} prefix={totalReturn >= 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />} valueStyle={{ color: totalReturn >= 0 ? "#059669" : "#dc2626" }} /></Card></Col>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="最大回撤" value={percentText(data?.max_drawdown)} prefix={<WarningOutlined />} valueStyle={{ color: "#dc2626" }} /></Card></Col>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="年化收益 CAGR" value={percentText(data?.cagr)} prefix={<RiseOutlined />} /></Card></Col>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="Sharpe 夏普比率" value={decimalText(data?.sharpe_ratio)} prefix={<BarChartOutlined />} /></Card></Col>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="胜率" value={percentText(data?.win_rate)} prefix={<FundProjectionScreenOutlined />} /></Card></Col>
          <Col xs={24} md={8} xl={4}><Card><Statistic title="已平仓交易数" value={data?.num_trades ?? "--"} prefix={<SwapOutlined />} /></Card></Col>
        </Row>

        <Card title="收益对比（策略 vs 买入持有）" extra={<Tag color={excessReturn >= 0 ? "green" : "orange"}>超额收益 {percentText(excessReturn)}</Tag>}>
          <Row gutter={[16, 16]}>
            <Col xs={24} md={8}><Statistic title="策略收益" value={percentText(data?.total_return)} valueStyle={{ color: totalReturn >= 0 ? "#059669" : "#dc2626" }} /></Col>
            <Col xs={24} md={8}><Statistic title="买入持有 Benchmark" value={percentText(data?.benchmark_return)} /></Col>
            <Col xs={24} md={8}><Statistic title="初始资金" value={moneyText(data?.initial_capital)} /></Col>
          </Row>
        </Card>
        ...
        <Card title={<Space><WarningOutlined />回撤曲线：从历史高点下跌幅度</Space>}>
          {drawdownPoints.length ? <LineSvg points={drawdownPoints} color="#dc2626" fill="#dc2626" valueLabel={(value) => percentText(value)} /> : <Typography.Text type="secondary">暂无回撤曲线数据</Typography.Text>}
        </Card>

        <Card title="完整分析指标说明">
          <Descriptions bordered column={{ xs: 1, md: 2, xl: 3 }} size="small">
            <Descriptions.Item label="Sortino 索提诺比率">{decimalText(data?.sortino_ratio)}</Descriptions.Item>
            <Descriptions.Item label="Calmar 卡玛比率">{decimalText(data?.calmar_ratio)}</Descriptions.Item>
            <Descriptions.Item label="Profit Factor 盈亏比">{decimalText(data?.profit_factor)}</Descriptions.Item>
            <Descriptions.Item label="滑点设置">{percentText(data?.slippage_pct)}</Descriptions.Item>
            <Descriptions.Item label="手续费设置">{percentText(data?.commission_pct)}</Descriptions.Item>
            <Descriptions.Item label="执行耗时">{data?.execution_time_ms == null ? "--" : `${data.execution_time_ms} ms`}</Descriptions.Item>
            <Descriptions.Item label="回测区间">{data?.start_date ?? "--"} 至 {data?.end_date ?? "--"}</Descriptions.Item>
            <Descriptions.Item label="标的 ID">{data?.stock_id ?? "--"}</Descriptions.Item>
            <Descriptions.Item label="策略 ID">{data?.config_id ?? "--"}</Descriptions.Item>
          </Descriptions>
        </Card>
        ...
        <Card title="交易明细（买入/卖出与单笔盈亏）">
          <Table<TradeRecord>
            ...
            columns={[
              ...
              { title: "成交价", dataIndex: "price", render: moneyText },
              { title: "单笔盈亏", dataIndex: "pnl", render: (value) => value == null ? "--" : <span className={Number(value) >= 0 ? "text-emerald-600" : "text-rose-600"}>{moneyText(value)}</span> },
            ]}
          />
        </Card>
      </Space>
