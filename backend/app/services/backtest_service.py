...
class BacktestService:
    ...
    async def run_backtest(
        ...
        try:
            ...
            strategy_frame = generate_strategy_frame(df, config)
            metrics = self._simulate(df, strategy_frame, initial_capital, slippage_pct, commission_pct)
    ...
    def _simulate(self, df: pd.DataFrame, signals: pd.Series | pd.DataFrame, initial_capital: float, slippage_pct: float, commission_pct: float) -> dict:
        if isinstance(signals, pd.DataFrame) and "target_position" in signals.columns and bool(signals["target_position"].notna().any()):
            return self._simulate_target_positions(df, signals, initial_capital, slippage_pct, commission_pct)

        signal_series = pd.Series(signals["signal"] if isinstance(signals, pd.DataFrame) else signals, index=df.index).fillna(0)
        ...
        for current_date, row in df.iterrows():
            ...
            signal = int(signal_series.loc[current_date]) if current_date in signal_series.index else 0
    ...
    def _simulate_target_positions(self, df: pd.DataFrame, frame: pd.DataFrame, initial_capital: float, slippage_pct: float, commission_pct: float) -> dict:
        cash = initial_capital
        shares = 0.0
        trades = []
        equity = []
        ...
        targets = pd.Series(frame["target_position"], index=df.index).ffill().fillna(0).astype(float).clip(-1, 1)
        ...
        for current_date, row in df.iterrows():
            ...
            target = float(targets.loc[current_date])
            desired_exposure = equity_before * target
            current_exposure = shares * price
            delta_exposure = desired_exposure - current_exposure
            reason = frame.loc[current_date].get("reason") if current_date in frame.index and "reason" in frame.columns else None
            confidence = frame.loc[current_date].get("confidence") if current_date in frame.index and "confidence" in frame.columns else None

            if abs(delta_exposure) > max(equity_before * 0.0001, 1e-6):
                fill = price * (1 + slippage_pct) if delta_exposure > 0 else price * (1 - slippage_pct)
                shares_delta = delta_exposure / fill
                notional = abs(shares_delta * fill)
                commission = notional * commission_pct
                cash -= shares_delta * fill + commission
                shares += shares_delta
                side = "buy" if shares_delta > 0 else "sell"
                equity_after_trade = cash + shares * price
                trade_pnl = equity_after_trade - previous_equity
                if trade_pnl >= 0:
                    wins += 1
                ...
                trades.append({
                    ...
                    "confidence": None if confidence is None or pd.isna(confidence) else round(float(confidence), 4),
